...
def build(target: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="enmotion-sidecar-") as temporary_name:
        ...
        worker_command = [
            ...
            "--collect-data",
            "demucs",
            # EnMotion invokes the fixed htdemucs WAV path. Avoid collecting
            # Hugging Face's optional client and unrelated tooling. Demucs'
            # lightweight ``hf`` adapter remains because its fallback path
            # imports that module before trying the legacy model host;
            # ``--collect-all demucs`` previously pulled PIL, cryptography,
            # mypy, Redis, and hf_xet into the on-demand worker.
            "--exclude-module",
            "huggingface_hub",
            "--exclude-module",
            "hf_xet",
            ...
            # Demucs uses eager tensor inference only. Compiler, ONNX,
            # functional-transform, symbolic-math, graph, and presentation
            # toolchains are not reachable from the fixed worker contract.
            "--exclude-module",
            "torch._dynamo",
            "--exclude-module",
            "torch._inductor",
        ...
        command = [
            ...
            "--exclude-module",
            "soundfile",
            # The native shell always runs the hybrid workspace API. Server
            # database/broker dependencies and development-only presentation,
            # reload, static-analysis, and array toolchains must stay out of
            # its launch-critical runtime.
            "--exclude-module",
            "sqlalchemy",
            "--exclude-module",
            "psycopg",
