...
- EnMotion ≠ "最好的剧本写作工具"（那是 Laper 的目标）
- EnMotion = "唯一能让剧本自动变成视频的编辑器"
...
Laper 的"CJK 切换"是二元开关。EnMotion 将其**升级为两维正交系统**：
...
| 2026-07-03 | 角色输入 = @呼出为主 + 冒号触发 + Tab兼容 | 与 EnMotion 视频生成流程一致的 @ 交互 |
