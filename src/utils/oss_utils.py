...
DEFAULT_OSS_BASE_PATH = "enmotion"
...
def authoritative_media_reference(
    local_path: str,
    output_root: str,
    mirrored_reference: Optional[str] = None,
) -> str:
    """Choose the persisted media reference after an optional OSS mirror.

    Portable server backups must remain self-contained, so server metadata
    always records the workspace-local file. OSS is an acceleration/mirror,
    not the authoritative copy. Desktop mode preserves the existing behavior
    of preferring a successfully uploaded object key.
    """

    root = Path(output_root).expanduser().resolve()
    local = Path(local_path).expanduser().resolve()
    if local != root and root not in local.parents:
        raise ValueError("Local media path escapes the configured output root")
    ...
    if server_mode_enabled():
        return relative
    return mirrored_reference or relative


def _server_workspace_prefix(base_path: str) -> Optional[str]:
    from ..apps.server.config import server_mode_enabled
    ...
    return f"{base_path}/workspaces/{tenant.workspace_id}"
...
class OSSImageUploader:
    ...
    def _build_object_key(self, sub_path: str, filename: str) -> str:
        ...
        Example: enmotion/proj_123/assets/characters/char_001.png
        """
        prefix = _server_workspace_prefix(self.base_path)
        parts = [prefix or self.base_path]
        if sub_path:
            parts.append(sub_path.strip("/"))
        parts.append(filename)
        return "/".join(parts)

    def _object_key_allowed(self, object_key: str) -> bool:
        prefix = _server_workspace_prefix(self.base_path)
        if prefix is None:
            return True
    ...
    def upload_file(self, local_path: str, sub_path: str = "", custom_filename: str = None) -> Optional[str]:
        ...
        Returns:
            Object Key (e.g., "enmotion/proj_123/assets/characters/file.png") or None if failed
        """
    ...
    def generate_signed_url(self, object_key: str, expires: int = SIGN_URL_EXPIRES_DISPLAY) -> str:
        ...
        if not self.bucket:
            ...
            return ""
        if not self._object_key_allowed(object_key):
            logger.warning("Refused to sign an OSS key outside the active workspace")
            return ""
    ...
    def object_exists(self, object_key: str) -> bool:
        ...
        if not self.bucket or not self._object_key_allowed(object_key):
            return False
        try:
...
def convert_local_path_to_object_key(local_path: str, project_id: str = None) -> str:
    ...
    Example:
        "assets/characters/char_001.png" -> "enmotion/proj_123/assets/characters/char_001.png"
    """
    ...
    workspace_prefix = _server_workspace_prefix(base_path)
    if workspace_prefix:
        base_path = workspace_prefix
