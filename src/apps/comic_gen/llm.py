...
def _resolve_image_for_vision(url: str, output_root: Optional[str] = None) -> Optional[str]:
    ...
    if server_mode_enabled():
        ...
        if is_object_key(s):
            s = OSSImageUploader().sign_url_for_api(s)
            if not s:
                return None
        if s.startswith("data:"):
            decode_image_data_url(s)
            return s
        if s.startswith(("http://", "https://")):
            return validate_remote_media_url(s)
        abs_path = resolve_workspace_media_path(current_workspace_output_root(), s)
        if os.path.getsize(abs_path) > image_limit_bytes():
            raise ValueError("Image is larger than the allowed limit")
...
# Placeholders: {ASSETS} = asset context, {DRAFT} = draft prompt
...
class ScriptProcessor:
    ...
    def split_into_episodes(self, text: str, suggested_episodes: int = 1) -> List[Dict[str, Any]]:
        """
