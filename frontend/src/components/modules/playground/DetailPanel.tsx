...
export default function DetailPanel({
  ...
  const handleGenerateVideo = () => {
    if (!output?.media_path || !onGenerateVideo) return;
    onGenerateVideo(output.media_path);
    onClose();
  };
  ...
  if (params.size) paramEntries.push([t('parameters.imageSize'), params.size]);
  if (params.resolution) paramEntries.push([t('parameters.resolution'), params.resolution]);
  if (params.aspect_ratio) paramEntries.push([t('parameters.aspectRatio'), params.aspect_ratio]);
  if (params.duration) paramEntries.push([t('parameters.duration'), `${params.duration}秒`]);
  if (generation.batch_size > 1)
    paramEntries.push([t('parameters.batchSize'), String(generation.batch_size)]);
  if (params.seed !== undefined && params.seed !== null)
    paramEntries.push([tui('randomSeed'), String(params.seed)]);
          ...
          {mediaUrl ? (
            isVideo ? (
              ...
              <PreviewImage
                ...
                diagnosticContext="playground-detail-result"
              />
            )
          ) : (
            <div className="flex flex-col items-center gap-2 text-text-muted">
              <Video className="w-12 h-12" />
              <span className="font-mono text-xs">{tui('noMedia')}</span>
            </div>
          )}
        ...
        <div className="relative w-[40%] h-full overflow-y-auto border-l border-border-subtle">
            ...
            <div>
              ...
              <div className="rounded-[14px] bg-surface-inset border border-border-subtle p-4 max-h-48 overflow-y-auto">
                <p className="font-display italic text-[0.9375rem] text-text-secondary leading-relaxed whitespace-pre-wrap break-words">
                  {generation.prompt ? `“${generation.prompt}”` : tui('emptyPrompt')}
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="max-h-28 overflow-y-auto rounded-[16px] bg-status-failed-bg border border-status-failed-border p-4">
                  <p className="text-[0.6875rem] text-status-failed-fg leading-relaxed break-all font-mono">
                    {/[A-Za-z]{2,}/.test(generation.error) ? t('card.failed') : generation.error}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="border-t border-border-subtle px-6 py-4 space-y-2.5">
            ...
            >
              ...
              {deleting ? tui('deleting') : tui('delete')}
            </button>
          </div>
        </div>
      </div>
