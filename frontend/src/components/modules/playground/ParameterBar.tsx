...
// ---------------------------------------------------------------------------
// ParamDropdown — native control with the established visual treatment.
// Native semantics keep keyboard, touch, and assistive technology behavior
// reliable without duplicating a complete listbox implementation.
// ---------------------------------------------------------------------------
...
function ParamDropdown({
  ...
  return (
    <div className="flex flex-col gap-[6px]">
      <label htmlFor={selectId} className="font-mono text-[0.625rem] uppercase tracking-[0.08em] text-text-muted">{label}</label>
      <div className="relative">
        <select
          ...
          className={`w-full appearance-none rounded-[14px] border border-border-subtle bg-surface-inset px-3 py-2.5 pr-9 text-xs font-medium text-foreground transition ${
            disabled
        ...
        <ChevronDown aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-text-muted" />
      </div>
    </div>
  );
}
...
function PillToggle({
  ...
  return (
    <div className="flex flex-col gap-[6px]">
      ...
      <div role="group" aria-label={label} className="flex gap-[2px] p-[3px] bg-surface-inset rounded-full atelier-pill-tabs">
        <button
          type="button"
          aria-pressed={value}
  ...
  return (
    <div className="flex flex-col gap-[6px]">
      ...
      <div className="flex items-center gap-0 rounded-[14px] border border-border-subtle bg-surface-inset overflow-hidden">
        <button
          type="button"
          aria-label={`${t('parameters.duration')}：减少`}
          disabled={value <= min}
        ...
        <div className="flex-1 flex items-center justify-center gap-0.5 py-2.5">
          ...
          />
          <span className="text-[0.625rem] text-text-muted font-mono">{t('parameters.secondsUnit')}</span>
        </div>
        <button
          type="button"
          aria-label={`${t('parameters.duration')}：增加`}
          disabled={value >= max}
      ...
      <div className="flex justify-between px-1">
        <span className="text-[0.5625rem] text-text-muted font-mono">{min}秒</span>
        <span className="text-[0.5625rem] text-text-muted font-mono">{max}秒</span>
      </div>
    </div>
  );
}
...
export default function ParameterBar() {
  ...
  const batchPills = (
    <div className="flex flex-col gap-[6px]">
      ...
      <div role="group" aria-label={t('parameters.batchSize')} className="flex gap-[2px] p-[3px] bg-surface-inset rounded-full atelier-pill-tabs">
        {BATCH_OPTIONS.map((n) => (
          <button
            key={n}
            type="button"
          ...
          <>
            ...
            {hasQuality && (
              <ParamDropdown
                ...
                formatOption={(quality) => t(`parameters.qualityOptions.${quality}`)}
              />
            )}
            ...
            {durationFixed ? (
              <div className="flex flex-col gap-[6px]">
                ...
                <div className="w-full flex items-center px-3 py-2.5 rounded-[14px] bg-surface-inset border border-border-subtle text-text-muted text-xs font-medium">
                  {t('parameters.secondsShort', { count: durationValue })} {t('parameters.durationFixedSuffix')}
                </div>
              </div>
            ) : modelDuration?.type === 'buttons' ? (
              <div className="flex flex-col gap-[6px]">
                <span className="font-mono text-[0.625rem] uppercase tracking-[0.08em] text-text-muted">{t('parameters.duration')}</span>
                <div role="group" aria-label={t('parameters.duration')} className="flex gap-[2px] p-[3px] bg-surface-inset rounded-full atelier-pill-tabs">
                  {modelDuration.options.map((n) => (
                    <button
                      key={n}
                      type="button"
                      aria-pressed={durationValue === n}
                    ...
                    >
                      {t('parameters.secondsShort', { count: n })}
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div>
          ...
          {showAdvanced && (
            <div className="grid grid-cols-2 gap-3 mt-3">
              {supportsSeed && (
                <label className="flex flex-col gap-[6px] atelier-field">
                  <span className="font-mono text-[0.625rem] uppercase tracking-[0.08em] text-text-muted">{tui('randomSeed')}</span>
                  <input
                    type="number"
