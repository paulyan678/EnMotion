  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="true"
        aria-controls="playground-queue-panel"
        onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <div id="playground-queue-panel" role="region" aria-label={t('queue.label')} className="absolute right-0 top-full z-40 mt-2 w-[min(300px,calc(100vw-2rem))] rounded-[16px] border border-glass-border bg-elevated p-4 shadow-2xl">
          {/* Concurrency control */}
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                aria-label={`${t('queue.concurrency')}：减少`}
                onClick={() => setMaxConcurrent(maxConcurrent - 1)}
                disabled={maxConcurrent <= 1}
                className="flex h-10 w-10 items-center justify-center rounded-md border border-glass-border bg-surface-inset text-text-muted transition-colors hover:text-foreground disabled:opacity-30"
              >
              ...
              <button
                type="button"
                aria-label={`${t('queue.concurrency')}：增加`}
                onClick={() => setMaxConcurrent(maxConcurrent + 1)}
                ...
                >
                  ...
                  {q.status === 'pending' && (
                    <button
                      ...
                      className="flex h-10 w-10 shrink-0 items-center justify-center rounded text-text-muted transition-colors hover:bg-status-failed-bg hover:text-status-failed-fg"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  )}
