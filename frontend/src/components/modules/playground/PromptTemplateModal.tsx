...
export default function PromptTemplateModal() {
  ...
  return (
    <ModalPortal isOpen={showTemplateModal} onClose={dismiss}>
      {(dialogRef) => (
    ...
    <div className="fixed inset-0 z-50" onClick={dismiss}>
      <div
        ...
        className="fixed right-0 top-0 flex h-[100dvh] max-h-[100dvh] w-full max-w-[420px] flex-col border-l border-glass-border bg-elevated shadow-2xl transition-transform duration-250 ease-out"
        style={{ transform: visible ? "translateX(0)" : "translateX(100%)" }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Header ── */}
        <div className="flex shrink-0 items-center gap-2 border-b border-border-subtle px-4 py-4 sm:px-6 sm:py-5">
          {view === "create" && (
            <button
              type="button"
              onClick={() => setView("browse")}
              aria-label={tc("back")}
              className="-ml-1.5 flex h-10 w-10 items-center justify-center rounded-lg text-text-muted transition-colors hover:bg-hover-bg hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
          )}
          <h2 id="prompt-template-dialog-title" className="atelier-display font-display text-[1.375rem] font-semibold tracking-tight text-foreground">
          ...
          <button
            ...
            onClick={dismiss}
            aria-label={tc("close")}
            className="ml-auto flex h-10 w-10 items-center justify-center rounded-lg text-text-muted transition-colors hover:bg-hover-bg hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {view === "browse" ? (
          <>
            ...
            {/* ── Template list ── */}
            <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-4 py-2 sm:px-6">
              {filtered.length === 0 ? (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex shrink-0 gap-1 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
                          <button
                            ...
                            className={`flex h-9 w-9 items-center justify-center rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 ${
                          ...
                          <button
                            ...
                            className="flex h-9 items-center gap-1 rounded-md bg-primary/10 px-2.5 text-[0.6875rem] font-medium text-primary transition-colors hover:bg-primary/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
                          >
                            <Copy size={11} />
                            {t("template.apply")}
                          </button>
                          <button
                            ...
                            className="flex h-9 w-9 items-center justify-center rounded-md text-text-muted transition-colors hover:bg-status-failed-bg hover:text-status-failed-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 disabled:opacity-30"
                          >
                            <Trash2 size={12} />
                          </button>
                        </div>
            ...
            {/* ── Footer: new template ── */}
            <div className="flex shrink-0 flex-wrap items-center gap-3 border-t border-border-subtle px-4 py-3 sm:px-6">
              <button
                type="button"
                onClick={openCreate}
                className="inline-flex min-h-10 items-center gap-1.5 rounded-lg px-2 text-xs font-medium text-text-muted transition-colors hover:bg-hover-bg hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
              >
                <Plus size={14} />
              ...
              {currentPrompt.trim() && (
                <button
                  ...
                  className="ml-auto inline-flex min-h-10 items-center gap-1.5 rounded-lg px-2 text-[0.6875rem] font-medium text-text-muted transition-colors hover:bg-hover-bg hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
                >
                  <Copy size={11} />
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto px-4 py-4 sm:px-6">
              ...
              {/* Prompt */}
              <textarea
                aria-label={t("template.promptPlaceholder")}
                value={form.prompt}
            ...
            <div className="shrink-0 border-t border-border-subtle px-4 py-4 sm:px-6">
              <button
                ...
                className={[
                  "min-h-11 w-full rounded-full text-[0.8125rem] font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60",
