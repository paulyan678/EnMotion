...
export default function AssetPickerModal({
  ...
  const dialogRef = useModalFocusTrap<HTMLDivElement>(onClose, isOpen);
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {/* Header                                                          */}
            {/* -------------------------------------------------------------- */}
              <div className="flex shrink-0 items-center justify-between border-b border-glass-border px-4 py-4 sm:px-6 sm:py-5">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-primary/15 flex items-center justify-center">
                  <Image size={16} className="text-primary" />
                </div>
                <h2 id="asset-picker-title" className="text-[0.9375rem] font-semibold text-foreground">{t('assetPicker.title')}</h2>
              </div>

              <button
                type="button"
                onClick={onClose}
                aria-label={t('assetPicker.close')}
            ...
            {visibleTabs.length > 1 && (
              <div className="flex shrink-0 items-center gap-1.5 px-4 pb-2 pt-4 sm:px-6">
                {visibleTabs.map((tab) => (
                  <button
                    key={tab.key}
                    type="button"
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {filteredAssets.map((asset) => {
                    ...
                    const thumbUrl = asset.thumbnail
                      ? getAssetUrl(asset.thumbnail)
                      : getAssetUrl(asset.path);
                      ...
                      >
                        ...
                        {asset.type === 'video' ? (
                          ...
                          <PreviewImage
                            ...
                            diagnosticContext="playground-asset-picker"
                          />
                        )}
