...
export function frameToShotNode(
    ...
    let videoStatus: "pending" | "processing" | "completed" | "failed" | "canceled" | undefined;
    ...
    if (inFlightTask) {
        ...
        videoStatus = "failed";
    } else if (frameTasks.some((t: any) => t.status === "canceled")) {
    ...
    return migrateShotNode({
        ...
        videoStatus,
        videoTaskId,
        imageUrl: selectedStoryboardImage(frame)?.url,
