...
export interface ShotNode {
    ...
    videoUrl?: string;
    videoTaskId?: string;
    videoStatus?: "pending" | "processing" | "completed" | "failed" | "canceled";
...
export default function ShotCard({
    ...
    const castAvatars = useCallback((): Array<{ id: string; name: string; avatarUrl?: string }> => {
        ...
        while ((match = tagPattern.exec(shot.prompt)) !== null) {
            ...
            seen.add(char.id);
            const avatarUrl = primaryAssetDisplayUrl(char, "character") || undefined;
    ...
    const renderPreview = () => {
        ...
        if (shot.videoStatus === "canceled") {
            return (
                <div className="w-full aspect-video flex items-center justify-center">
                    <span className="text-[0.6875rem] text-text-muted font-medium">{t("canceledMessage")}</span>
                </div>
            );
        }
        if (shot.t2iImageUrl) {
            return (
                <div className="w-full aspect-video relative">
                    <PreviewImage src={shot.t2iImageUrl} alt={t("t2iCompleted")} className="w-full h-full" />
    ...
    const handleInsertAssetFromChip = (_type: string, name: string, _ref: AssetRef) => {
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                document.dispatchEvent(
                                                    new CustomEvent("enmotion:navigateStep", { detail: "cast" }),
                                                );
                                            }}
                                            title={c.name}
                                            ...
                                            <div className="mt-1.5 rounded-md border border-glass-border bg-black/20 px-3 py-2 text-[0.71875rem] leading-relaxed font-mono space-y-2">
                                                ...
                                                {shot.duration && (
                                                    <p className="text-text-muted border-t border-border-subtle pt-1.5">
                                                        <span className="text-status-completed-fg/70">{t("durationLabel")}：</span> {shot.duration}秒 {t("durationApiNote")}
                                                    </p>
                                                )}
                                            </div>
                                        </motion.div>
                ...
                <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-t border-border-subtle">
                            <div className="flex items-center gap-1 shrink-0">
                                ...
                                <motion.button
                                    ...
                                    className="ico-btn flex h-8 w-8 items-center justify-center rounded-[14px] text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-status-failed-fg disabled:cursor-wait disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                    title={t("deleteShot")}
                                >
                                    {isDeleting
                                    ...
                                    {[1, 2, 4, 6].map((n) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                aria-pressed={active}
                                                aria-label={t("genCandidatesEachTooltip", { n })}
