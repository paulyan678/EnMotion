...
type TabKey = "active" | "done" | "failed" | "canceled";
...
export default function TaskQueuePanel({
    ...
    const buckets = useMemo(() => {
        ...
        for (const t of tasks) {
            ...
            else if (t.status === "canceled") canceled.push(t);
        }
        ...
        canceled.sort(byTimeDesc);
        return { active, done, failed, canceled };
    }, [tasks]);
            ...
            <div role="tablist" className="flex shrink-0 border-b border-glass-border px-3 py-1.5">
                {(
                    [
                        ...
                        ["canceled", "queueCanceled", buckets.canceled.length, "text-text-muted"],
...
function TaskRow({
    ...
    const elapsedLabel = elapsedS < 5
        ? t("batchAgeNow")
        : elapsedS < 60
        ? t("batchAgeSeconds", { s: elapsedS })
        : elapsedS < 3600
            ? t("batchAgeMinutes", { m: Math.floor(elapsedS / 60) })
            : t("batchAgeHours", { h: Math.floor(elapsedS / 3600) });
    ...
    const providerLabel = t("providerLabel");
    const failureMessage = (() => {
        switch (task.error_code) {
            case "input_image_privacy": return t("errors.inputImagePrivacy");
            case "video_generation_timeout": return t("errors.timeout");
            case "video_generation_interrupted": return t("errors.interrupted");
            case "video_queue_unavailable": return t("errors.queueUnavailable");
            case "video_generation_failed": return t("errors.generic");
            default:
                return task.error && (locale !== "zh" || !/[A-Za-z]{2,}/.test(task.error))
                    ? task.error
                    : t("unknownErrorFallback");
        }
    })();

    // Build diagnose blob — copy-pasteable into a support ticket.
    const diagnoseBlob = [
        `${t("localIdLabel")} ${task.id}`,
        task.provider_name && task.provider_task_id
            ? `${providerLabel} ${t("taskIdLabel")} ${task.provider_task_id}`
            : null,
        task.provider_request_id
            ? `${providerLabel} ${t("requestIdLabel")} ${task.provider_request_id}`
            : null,
        `${t("model")}：${modelDisplayName(task.model)}`,
        `${t("configuredStatus")}：${task.status}`,
        task.error ? `${t("errorLabel")}：${failureMessage}` : null,
        task.error_diagnostic ? `${t("technicalDetails")}：${task.error_diagnostic}` : null,
    ].filter(Boolean).join("\n");
        ...
        >
            ...
            <div className="flex items-center gap-1.5">
                <button
                    ...
                    aria-label={expanded ? t("queueCollapse") : t("queueExpandDetails")}
            ...
            {!expanded ? (
                ...
                <div className="flex items-start gap-2">
                    <div className="h-[36px] w-[64px] shrink-0 overflow-hidden rounded border border-glass-border bg-black/40">
                        {isCompleted && outputVideoUrl ? (
                            <PreviewVideo src={outputVideoUrl} alt={t("output")} className="h-full w-full" hoverPlay={false} alwaysShowMagnify clickToLightbox />
                        ) : inputThumbUrl ? (
                            <PreviewImage src={inputThumbUrl} alt={t("input")} className="h-full w-full" alwaysShowMagnify clickToLightbox />
                        ) : (
                ...
                <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap items-start gap-2">
                            {inputThumbUrl ? (
                                <div className="space-y-0.5">
                                    <p className="font-mono text-[0.5625rem] uppercase tracking-wider text-text-muted">{t("input")}</p>
                            ...
                            {isCompleted && outputVideoUrl ? (
                                <div className="space-y-0.5">
                                    <p className="font-mono text-[0.5625rem] uppercase tracking-wider text-text-muted">{t("output")}</p>
                                    <div className="h-[68px] w-[120px] overflow-hidden rounded border border-glass-border bg-black/40">
                                        <PreviewVideo src={outputVideoUrl} alt={t("output")} className="h-full w-full" alwaysShowMagnify clickToLightbox />
                                    </div>
                                </div>
                            ) : null}
                        </div>
                    ...
                    {fullPrompt ? (
                        <div className="space-y-0.5">
                            <p className="font-mono text-[0.5625rem] uppercase tracking-wider text-text-muted">{t("promptLabel")}</p>
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 font-mono text-chrome-sm text-text-muted">
                        <span className="text-text-secondary">{modelDisplayName(task.model)}</span>
                        {task.resolution ? <span>· {task.resolution}</span> : null}
                        {task.duration ? <span>· {task.duration} {t("durationUnit")}</span> : null}
                        {typeof task.seed === "number" ? <span>· {t("seedLabel")} {task.seed}</span> : null}
                    ...
                    {isFailed ? (
                        <div className="space-y-0.5">
                            <p className="font-mono text-[0.5625rem] uppercase tracking-wider text-status-failed-fg/80">{t("errorLabel")}</p>
                            ...
                            {task.error_diagnostic ? (
                                <details className="rounded border border-glass-border/60 bg-black/20 px-2 py-1.5 text-text-muted">
                                    <summary className="cursor-pointer font-mono text-chrome-sm hover:text-foreground">{t("technicalDetails")}</summary>
                                    <pre className="mt-1.5 whitespace-pre-wrap break-words font-mono text-[0.625rem] leading-snug">
                                        {locale === "zh" ? t("diagnosticHidden") : task.error_diagnostic}
                                    </pre>
                                </details>
                            ) : null}
                        </div>
                    ...
                    {isCanceled ? (
                        <p className="rounded border border-glass-border/60 bg-black/20 px-2 py-1.5 font-mono text-chrome-sm text-text-muted">
                            {t("canceledMessage")}
                        </p>
                    ) : null}
                    ...
                    {task.provider_task_id || task.provider_request_id ? (
                        <div className="space-y-1 rounded border border-glass-border/60 bg-black/30 px-2 py-1.5">
                            <p className="font-mono text-[0.5625rem] uppercase tracking-wider text-text-muted">{t("identifierGroup", { provider: providerLabel })}</p>
                            {task.provider_task_id ? (
                                <div className="flex items-center gap-1.5">
                                    <span className="font-mono text-chrome-sm text-text-muted">{t("taskIdLabel")}</span>
                            ...
                            {task.provider_request_id ? (
                                <div className="flex items-center gap-1.5">
                                    <span className="font-mono text-chrome-sm text-text-muted">{t("requestIdLabel")}</span>
                                    ...
                                    <button
                                        ...
                                        aria-label={t("copyRequestIdAria")}
                                        className="-m-1 grid h-6 w-6 place-items-center rounded text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                    >
