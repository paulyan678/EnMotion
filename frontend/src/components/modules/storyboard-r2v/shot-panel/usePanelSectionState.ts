...
function readState(shotId: string, section: string, defaultOpen: boolean): boolean {
    ...
    try {
        const raw = readWorkspaceItem(key(shotId, section));
...
export function usePanelSectionState(
    ...
    const set = useCallback((next: boolean) => {
        ...
        if (typeof window === "undefined") return;
        try {
            writeWorkspaceItem(key(shotId, section), next ? "1" : "0");
        } catch {
            /* ignore */
        }
    }, [shotId, section]);
...
export const PANEL_SECTION_OVERRIDE_EVENT = "enmotion:panel-section-override";
    ...
    try {
        for (const sid of shotIds) {
            for (const sec of sections) {
                writeWorkspaceItem(key(sid, sec), open ? "1" : "0");
            }
        }
    } catch {
