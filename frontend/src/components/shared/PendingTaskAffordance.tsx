...
function formatElapsed(ms: number, t: ReturnType<typeof useTranslations>): string {
    const s = Math.max(0, Math.floor(ms / 1000));
    if (s < 60) return t("seconds", { seconds: s });
    const m = Math.floor(s / 60);
    const r = s % 60;
    return t("minutesSeconds", { minutes: m, seconds: r });
}
...
export function PendingTaskAffordance({
    ...
    const elapsedLabel = formatElapsed(elapsedMs, t);
    ...
    const handleCancel = async () => {
        ...
        try {
            ...
            setCancelError(locale === "zh" ? t("cancelFailed") : (err instanceof Error ? err.message : t("cancelFailed")));
        } finally {
            setCanceling(false);
        }
    };
...
export function DiagnoseModal({ taskId, elapsedLabel, onClose }: DiagnoseModalProps) {
    ...
    const requestLog = useCallback(() => {
        api.diagnoseLogTail(200)
            ...
            .catch((err) =>
                setLog({ kind: "error", message: locale === "zh" ? t("readLogFailed") : (err instanceof Error ? err.message : String(err)) }),
            );
    }, [locale, t]);
    ...
    useEffect(() => {
        ...
        api.healthCheck()
            ...
            .catch((err) => {
                if (!cancelled) setHealth({ kind: "error", message: locale === "zh" ? t("unreachable") : (err instanceof Error ? err.message : String(err)) });
            });
        requestLog();
        return () => {
            cancelled = true;
                ...
                <div className="space-y-3 overflow-y-auto px-4 py-4 text-[0.78125rem] leading-[1.55] text-text-secondary/95">
                    ...
                    <Row label={t("backend")}>
                        {health.kind === "loading" ? (
                            ...
                            <span className="font-mono text-emerald-300">{t("reachable", { count: health.data.studio_projects })}</span>
                        ) : (
                            <span className="font-mono text-red-300">{t("unreachable")}{locale === "zh" ? "" : ` · ${health.message}`}</span>
                        )}
                    </Row>
                    {health.kind === "ok" ? (
                        <Row label={t("logFile")}>
                    ...
                    <div className="rounded-md border border-glass-border bg-black/30">
                        ...
                        {log.kind === "ok" && log.data.errors.length > 0 ? (
                            <>
                                <div className="flex items-center justify-between gap-2 border-b border-red-400/15 bg-red-500/[0.08] px-3 py-1 font-mono text-[0.5625rem] font-medium uppercase tracking-[0.22em] text-red-200/95">
                                    ...
                                    <span className="text-red-200/70 normal-case tracking-tight">{t("rootCauseHint")}</span>
                                </div>
                        ...
                        <div className="max-h-[280px] overflow-y-auto px-3 py-2 font-mono text-[0.625rem] leading-[1.55] text-text-secondary/95">
                            {log.kind === "loading" ? (
                                ...
                                <div className="text-red-300">{t("readLogFailed")}{locale === "zh" ? "" : `: ${log.message}`}</div>
