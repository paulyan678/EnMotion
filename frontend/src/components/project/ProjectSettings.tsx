...
export default function ProjectSettings({ project, isOpen, onClose, onUpdate }: ProjectSettingsProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const errorMessage = locale === "zh" ? t("updateFailed") : (error?.response?.data?.detail || error?.message || t("updateFailed"));
            alert(locale === "zh" ? errorMessage : t("updateFailedDetail", { error: errorMessage }));
        } finally {
            setIsSaving(false);
        }
    };
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {STYLE_PRESETS.map((style) => (
                                        ...
                                        >
                                            <div className="font-semibold text-sm mb-1">{t(`stylePresets.${style.value}.label`)}</div>
                                            <div className="text-xs text-text-muted">{t(`stylePresets.${style.value}.description`)}</div>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            {selectedStyle && (
                                <div className="bg-glass border border-glass-border rounded-lg p-4">
                                    ...
                                    <p className="text-sm text-blue-400 italic">
                                        &quot;{t(`stylePresets.${selectedStyle.value}.label`)} {t("styleSuffix")}{stylePrompt ? `${locale === "zh" ? "，" : ", "}${stylePrompt}` : ""}&quot;
                                    </p>
                                </div>
                            )}
                        </div>
