                ...
                <AnimatePresence>
                    ...
                    >
                        <motion.div
                            ...
                            role="dialog"
                            aria-modal="true"
                            aria-labelledby="project-prompt-config-title"
                            tabIndex={-1}
                    ...
                    <div className="flex shrink-0 items-start justify-between gap-4 border-b border-glass-border p-4 sm:items-center sm:p-6">
                        <div className="flex min-w-0 items-center gap-3">
                            ...
                            <div className="min-w-0">
                                <h2 id="project-prompt-config-title" className="text-lg font-bold text-foreground">{t("promptConfig")}</h2>
                        ...
                        {isLoading ? (
                            ...
                            <>
                                ...
                                <div className="space-y-2">
                                    <div>
                                        <h3 id="project-polish-model-label" className="text-sm font-bold text-foreground">{t("polishModelTitle")}</h3>
                                        <p className="text-[0.625rem] text-text-muted mt-0.5">
                                            {t("polishModelDescription")}
                                        </p>
                                    </div>
                                    <select
                                        aria-labelledby="project-polish-model-label"
                                ...
                                {SECTIONS.map((section) => (
                                    <div key={section.key} className="space-y-2">
                                        <div className="flex flex-col items-start gap-2 sm:flex-row sm:justify-between">
                                            <div className="min-w-0">
                                                <h3 id={`project-${section.key}-label`} className="text-sm font-bold text-foreground">{t(section.labelKey)}</h3>
                                                <p className="text-[0.625rem] text-text-muted mt-0.5">{t(section.descriptionKey)}</p>
                                            </div>
                                            <button
                                                type="button"
                                        ...
                                        <textarea
                                            aria-labelledby={`project-${section.key}-label`}
                                            value={config[section.key]}
                                            onChange={(e) => setConfig(prev => ({ ...prev, [section.key]: e.target.value }))}
                                            placeholder={t("loadingDefault")}
                                            className="h-32 min-h-28 w-full resize-y rounded-lg border border-glass-border bg-surface p-3 font-mono text-xs text-foreground placeholder-text-muted focus:outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/40"
                                        />
                                        ...
                                        {defaults && (
                                            <div>
                                                <button
                                                    ...
                                                    aria-controls={`project-${section.key}-default`}
                                                ...
                                                {expandedDefault === section.key && (
                                                    <pre id={`project-${section.key}-default`} className="mt-2 max-h-48 overflow-x-auto overflow-y-auto whitespace-pre-wrap rounded-lg border border-border-subtle bg-surface p-3 font-mono text-[0.625rem] text-text-muted">{defaults[section.key]}</pre>
                                                )}
                                            </div>
                                        )}
