...
export default function SeriesSidebar({
  ...
  return (
    <motion.aside
      ...
      className="flex h-auto max-h-[42vh] w-full flex-shrink-0 flex-col border-b border-glass-border bg-surface backdrop-blur-xl md:h-full md:max-h-none md:w-64 md:border-b-0 md:border-r"
    >
      ...
      <div className="border-t border-glass-border p-3 space-y-1">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-status-failed-fg/80 hover:text-status-failed-fg hover:bg-status-failed-bg transition-colors group disabled:cursor-wait disabled:opacity-50"
        >
          <Trash2 size={16} />
          <span className="text-sm">
            {isDeletingSeries ? t("deletingSeries") : t("deleteSeries")}
          </span>
        </button>
      </div>
    </motion.aside>
