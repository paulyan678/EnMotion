...
function createMockFile(name: string = 'test.txt', type: string = 'text/plain') {
...
describe('ImportFileDialog', () => {
    ...
    describe('Step 1 - Upload', () => {
        it('shows file upload zone', () => {
            ...
            expect(screen.getByText(/支持 .txt \/ .md \/ .pdf 文件/)).toBeInTheDocument();
        });
        ...
        it('allows PDF files and auto-fills the title', () => {
            renderDialog();
            const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
            expect(fileInput).toHaveAttribute('accept', '.txt,.md,.pdf');

            const file = createMockFile('my-story.pdf', 'application/pdf');
            fireEvent.change(fileInput, { target: { files: [file] } });

            expect(screen.getByText('my-story.pdf')).toBeInTheDocument();
            expect(screen.getByPlaceholderText('输入系列标题...')).toHaveValue('my-story');
        });
        ...
        it('shows error for unsupported file types', () => {
            ...
            fireEvent.change(fileInput, { target: { files: [file] } });
            expect(screen.getByText('仅支持 .txt、.md 和 .pdf 文件')).toBeInTheDocument();
        });
    });
    ...
    describe('Step 1 → Step 2 transition', () => {
        ...
        it('calls API and transitions to step 2 on analyze', async () => {
            ...
            await waitFor(() => {
                expect(mockImportFilePreview).toHaveBeenCalledWith(expect.any(File), 1);
            });
            await waitFor(() => {
                expect(screen.getByText(/人工智能已将文件分为 2 集/)).toBeInTheDocument();
            });
        });
    ...
    describe('Step 2 → Step 3 transition', () => {
        ...
        it('opens the newly created series returned by the backend', async () => {
            const onClose = vi.fn();
            const onSuccess = vi.fn();
            mockImportFileConfirm.mockResolvedValue(mockCreatedResult);
            await goToStep2({ onClose, onSuccess });
