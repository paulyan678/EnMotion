...
interface ImportFileDialogProps {
    isOpen: boolean;
    onClose: () => void;
    onSuccess?: (result: CreatedImportResult) => void;
}
...
export default function ImportFileDialog({ isOpen, onClose, onSuccess }: ImportFileDialogProps) {
    ...
    const [suggestedEpisodes, setSuggestedEpisodes] = useState(1);
    ...
    // Step 3 state
    const [createdResult, setCreatedResult] = useState<CreatedImportResult | null>(null);
    ...
    const handleFileSelect = useCallback((selectedFile: File) => {
        ...
        if (!ext || !SUPPORTED_FILE_EXTENSIONS.has(ext)) {
            setError(t("unsupportedImportFile"));
            return;
        }
        ...
        setSeriesTitle((prev) => {
            if (!prev) {
                return selectedFile.name.replace(/\.(txt|md|pdf)$/i, '');
            }
            return prev;
        });
    }, [t]);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            setError(locale === "zh" ? t("analysisFailed") : msg);
        } finally {
            setIsAnalyzing(false);
        }
    };
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError(locale === "zh" ? t("createFailed") : msg);
        } finally {
            setIsCreating(false);
        }
    };
                                        ...
                                        {file ? (
                                            <div className="flex items-center justify-center gap-3">
                                                ...
                                                <div className="text-left">
                                                    ...
                                                    <p className="text-text-secondary text-sm">{t("fileSizeKb", { size: (file.size / 1024).toFixed(1) })}</p>
                                                </div>
                                            </div>
                                        ) : (
                                            <>
                                            ...
                                            >
                                                <div className="flex items-center gap-3 mb-2">
                                                    <span className="bg-primary/20 text-primary text-xs font-bold px-2 py-1 rounded-full">
                                                        {t("episodeNumber", { number: ep.episode_number })}
                                                    </span>
