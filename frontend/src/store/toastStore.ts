...
function cancelAutoClose(id: string): void {
    const timer = autoCloseTimers.get(id);
    if (timer) clearTimeout(timer);
    autoCloseTimers.delete(id);
}

function syncAutoClose(toast: Toast): void {
    cancelAutoClose(toast.id);
    // Failures are intentionally sticky so users can read, copy, and retry
    // them at their own pace, even if a previous toast state had a timer.
    if (toast.kind === "error" || !toast.autoCloseMs || toast.autoCloseMs <= 0) return;

    const timer = setTimeout(() => {
        autoCloseTimers.delete(toast.id);
        useToastStore.getState().dismiss(toast.id);
    }, toast.autoCloseMs);
    autoCloseTimers.set(toast.id, timer);
}
...
export const useToastStore = create<ToastStore>((set) => ({
    ...
    update: (id, patch) => {
        ...
        if (updatedToast) syncAutoClose(updatedToast);
    },
    dismiss: (id) => {
        cancelAutoClose(id);
    ...
    clear: () => {
        for (const id of autoCloseTimers.keys()) cancelAutoClose(id);
        set({ toasts: [] });
    },
}));
